...
const getMovements = async (req, res, next) => {
  try {
    ...
    if (startDate) {
      conditions.push("DATE(sm.createdAt) >= ?");
    ...
    const [[{ total }]] = await pool.execute(
      `SELECT COUNT(*) as total FROM stock_movements sm WHERE ${where}`,
      params
    );
...
const adjustStock = async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (newStock < 0) {
        throw new AppError(
          `Cannot reduce stock below 0. Current stock: ${product.stock}`, 400
        );
      }
      ...
      await conn.execute(
        ...
        [
          ...
          type, delta,
          reason || `Manual ${type.toLowerCase()}`,
...
// ─── BULK ADJUSTMENT (after physical count) ───────────
const bulkAdjust = async (req, res, next) => {
  try {
    ...
    try {
      ...
      for (const adj of adjustments) {
        ...
        await conn.execute(
          `INSERT INTO stock_movements
           ...
           VALUES (?,?,?,'ADJUSTMENT',?,?,?,?)`,
          [
            uuidv4(), adj.productId, req.user.user_id,
            delta,
            reason || "Physical stock count",
            product.stock, adj.newStock,
          ]
        );
        count++;
      }
      ...
      res.json({
        success: true,
        message: `${count} products adjusted successfully.`,
      });
    } catch (err) {
      await conn.rollback();
      throw err;
...
module.exports = { getMovements, adjustStock, bulkAdjust };