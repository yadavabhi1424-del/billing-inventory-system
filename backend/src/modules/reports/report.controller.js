  ...
  try {
    ...
    const [byPaymentMethod] = await pool.execute(
      `SELECT paymentMethod,
       ...
       FROM transactions
       WHERE createdAt BETWEEN ? AND ? AND status = 'COMPLETED'
    ...
    res.json({
      success: true,
      data: { summary, byPaymentMethod, topProducts, dailyBreakdown },
    });
  ...
  try {
    ...
    const [lowStockItems] = await pool.execute(
      `SELECT p.product_id, p.name, p.sku,
              ...
              p.sellingPrice, s.name as supplierName,
    ...
    const [categoryWise] = await pool.execute(
      `SELECT c.name as categoryName,
              ...
              SUM(p.stock * p.costPrice)   as stockValue
       FROM categories c
    ...
    res.json({
      success: true,
      data: { totals, lowStockItems, categoryWise },
    });
  } catch (error) { next(error); }
};
    ...
    const [topCustomers] = await pool.execute(
      `SELECT customer_id, name, phone,
              ...
              (SELECT COUNT(*) FROM transactions
               WHERE customer_id = c.customer_id) as totalOrders
       FROM customers c
...
const getProfitLoss = async (req, res, next) => {
  try {
    ...
    const profitMargin  = result.totalRevenue > 0
      ? ((grossProfit / result.totalRevenue) * 100).toFixed(2)
      : 0;
