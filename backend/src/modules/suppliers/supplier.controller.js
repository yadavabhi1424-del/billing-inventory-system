...
const getAllSuppliers = async (req, res, next) => {
  try {
    ...
    if (search) {
      conditions.push("(s.name LIKE ? OR s.phone LIKE ? OR s.email LIKE ?)");
  ...
  try {
    ...
    const [products] = await pool.execute(
      `SELECT product_id, name, sku, stock, sellingPrice
       FROM products
       WHERE supplier_id = ? AND isActive = TRUE
       ORDER BY name ASC`,
      [req.params.id]
    );
    ...
    res.json({
      success: true,
      data: { ...rows[0], products, recentOrders: orders },
    });
  } catch (error) {
    next(error);
  }
...
const createSupplier = async (req, res, next) => {
  try {
    const {
      ...
      gstin, bankName, bankAccount,
      ifscCode, paymentTerms, notes,
    } = req.body;
    ...
    await pool.execute(
      `INSERT INTO suppliers
        (supplier_id, name, contactPerson, email, phone,
         address, city, state, pincode, gstin,
         bankName, bankAccount, ifscCode, paymentTerms, notes)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        supplierId, name, contactPerson || null,
        email || null, phone,
        address || null, city || null,
        state || null, pincode || null,
        gstin || null, bankName || null,
        bankAccount || null, ifscCode || null,
        paymentTerms || "30 days", notes || null,
      ]
    );
...
const updateSupplier = async (req, res, next) => {
  try {
    const {
      ...
      address, city, state, pincode,
      gstin, bankName, bankAccount,
      ifscCode, paymentTerms, notes, isActive,
    } = req.body;
