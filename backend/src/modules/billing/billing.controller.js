    ...
    try {
      ...
      await conn.execute(
        ...
        [
          ...
          customerId || null, req.user.user_id,
          paymentMethod,
          paid >= totalAmount ? "PAID" : paid > 0 ? "PARTIAL" : "UNPAID",
          subtotal, discountType || null,
          parseFloat(discountValue), discountAmount,
          totalTax, roundOff, totalAmount,
      ...
      for (const item of enrichedItems) {
        ...
        await conn.execute(
          `INSERT INTO transaction_items
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          [
            item.itemId, transactionId, item.productId,
            item.productName, item.sku, item.quantity,
            item.unit, item.costPrice, item.sellingPrice,
        ...
        await conn.execute(
          `INSERT INTO stock_movements
           ...
           VALUES (?,?,?,'SALE',?,?,?,?,?)`,
          [
            uuidv4(), item.productId, req.user.user_id,
            -item.quantity, "POS Sale",
            invoiceNumber, item.stock, newStock,
          ]
        );
      }
      ...
      for (const p of paymentList) {
        await conn.execute(
          `INSERT INTO payments
            (payment_id, transaction_id, method, amount, reference)
           VALUES (?,?,?,?,?)`,
          [uuidv4(), transactionId, p.method,
           parseFloat(p.amount), p.reference || null]
        );
      }
...
const getAllTransactions = async (req, res, next) => {
  try {
    ...
    if (search) {
      conditions.push("(t.invoiceNumber LIKE ? OR c.name LIKE ?)");
...
const returnTransaction = async (req, res, next) => {
  try {
    ...
    if (transaction.status === "RETURNED") {
      return next(new AppError("Transaction already returned.", 400));
    }

    if (transaction.status === "CANCELLED") {
      return next(new AppError("Cannot return a cancelled transaction.", 400));
    }
    ...
    try {
      ...
      for (const item of items) {
        ...
        await conn.execute(
          `INSERT INTO stock_movements
           ...
           VALUES (?,?,?,'RETURN_IN',?,?,?,?,?)`,
          [
            uuidv4(), item.product_id, req.user.user_id,
            item.quantity,
      ...
      await conn.execute(
        `UPDATE transactions
         SET status = 'RETURNED', returnReason = ?, paymentStatus = 'REFUNDED'
      ...
      if (transaction.customer_id) {
        ...
        await conn.execute(
          `UPDATE customers SET
            totalSpent    = totalSpent - ?,
            loyaltyPoints = GREATEST(0, loyaltyPoints - ?)
           WHERE customer_id = ?`,
